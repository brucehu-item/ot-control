...
router.post('/overtime-requests', authenticate, (req: Request, res: Response) => {
...
router.get('/overtime-requests', authenticate, (req: Request, res: Response) => {
    const overtimeController = GlobalContainer.getInstance().get(OvertimeController);
    return overtimeController.findRequests(req, res);
});

// Get overtime request details
router.get('/overtime-requests/:requestId', authenticate, (req: Request, res: Response) => {
    const overtimeController = GlobalContainer.getInstance().get(OvertimeController);
    return overtimeController.getRequestDetails(req, res);
});

// Approve overtime request
router.post('/overtime-requests/:requestId/approve', authenticate, requireRole(['SUPERVISOR', 'MANAGER', 'CUSTOMER']), (req: Request, res: Response) => {
    const overtimeController = GlobalContainer.getInstance().get(OvertimeController);
    return overtimeController.approveRequest(req, res);
});

// Reject overtime request
router.post('/overtime-requests/:requestId/reject', authenticate, requireRole(['SUPERVISOR', 'MANAGER', 'CUSTOMER']), (req: Request, res: Response) => {
    const overtimeController = GlobalContainer.getInstance().get(OvertimeController);
    return overtimeController.rejectRequest(req, res);
});

// Edit overtime request
router.put('/overtime-requests/:requestId', authenticate, requireRole(['WORKER']), (req: Request, res: Response) => {
    const overtimeController = GlobalContainer.getInstance().get(OvertimeController);
